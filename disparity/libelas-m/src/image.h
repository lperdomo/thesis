...
image<uchar> *loadPGM(const char *name) {
  ...
  // read data
  image<uchar> *im = new image<uchar>(1241, 376);
  file.read((char *)imPtr(im, 0, 0), 1241 * height * sizeof(uchar));

  return im;
}

image<uchar> *loadPNG(const char *name) {
  cv::Mat test;
  test = imread(name);
  image<uchar> *im = test.data;
  return im;
}
