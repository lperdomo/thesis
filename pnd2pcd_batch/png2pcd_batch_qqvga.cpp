...
main ( int argc, char* argv[] )
{
    ...
    pcl::PointCloud<pcl::PointXYZRGBA>::Ptr down;
    ...
    for ( int i = 0; i < pcd_file_names.size(); i++)
    {
    	...
    	for(unsigned r=0;r<current_xyzrgb_cloud.height;r++)
    	{
	    for(unsigned c=0;c<current_xyzrgb_cloud.width;c++)
	    {
		if (0.3 > current_xyzrgb_cloud.at(c,r).z ||
		    current_xyzrgb_cloud.at(c,r).z > 5.0)
		{
		    //current_xyzrgb_cloud.at(c,r).x = std::numeric_limits<double>::quiet_NaN();
		    //current_xyzrgb_cloud.at(c,r).y = std::numeric_limits<double>::quiet_NaN();
		    current_xyzrgb_cloud.at(c,r).z = std::numeric_limits<double>::quiet_NaN();
		}
	    }
	}


    	int downsamplingStep = 4;

	down.reset(new pcl::PointCloud<pcl::PointXYZRGBA>());
	down->points.resize(current_xyzrgb_cloud.size()/(downsamplingStep*downsamplingStep));
	down->width = current_xyzrgb_cloud.width/downsamplingStep;
	down->height = current_xyzrgb_cloud.height/downsamplingStep;
	down->is_dense = false;
	...
	for(unsigned r=0;r<current_xyzrgb_cloud.height;r=r+downsamplingStep)
	{
	for(unsigned c=0;c<current_xyzrgb_cloud.width;c=c+downsamplingStep)
	{
	    unsigned nPoints=0;
	    ...
	    unsigned centerPatch = (r+downsamplingStep/2)*current_xyzrgb_cloud.width+c+downsamplingStep/2;
	    ...
	    for(unsigned r2=r;r2<r+downsamplingStep;r2++)
	    {
	        for(unsigned c2=c;c2<c+downsamplingStep;c2++)
	        {
	            ...
	            if(pcl_isfinite (current_xyzrgb_cloud.points[r2*current_xyzrgb_cloud.width+c2].x) &&
...
//                       pcl_isfinite (pointCloudPtr->points[r2*pointCloudPtr->width+c2].z) &&
	               0.3 < current_xyzrgb_cloud.points[r2*current_xyzrgb_cloud.width+c2].z &&
	               current_xyzrgb_cloud.points[r2*current_xyzrgb_cloud.width+c2].z < 5.0)
	            {
	                //Create a vector with the x, y and z coordinates of the square region
	                xV[nPoints]=current_xyzrgb_cloud.points[r2*current_xyzrgb_cloud.width+c2].x;
	                yV[nPoints]=current_xyzrgb_cloud.points[r2*current_xyzrgb_cloud.width+c2].y;
	                zV[nPoints]=current_xyzrgb_cloud.points[r2*current_xyzrgb_cloud.width+c2].z;
	    ...
	    if(nPoints>0)
	    {
	        ...
	        point.r = current_xyzrgb_cloud.points[centerPatch].r;
	        point.g = current_xyzrgb_cloud.points[centerPatch].g;
	        point.b = current_xyzrgb_cloud.points[centerPatch].b;
	    ...
	    else
	    {
	        ...
	        //Set a nan point to keep the m_DownsampledPointCloudPtr organised
	          //down->points[j] = current_xyzrgb_cloud.points[centerPatch];
	        pcl::PointXYZRGBA point;
        ...
        pcl::io::savePCDFileBinaryCompressed<pcl::PointXYZRGBA> ( pcd_file_names[i], *down );
    }
    return 0;
}
