...
std::vector<std::vector<float> > COSPAIR::ComputeCOSPAIR(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud, double R, int levels, int bins, int rgb_type, int rgb_bins, int keypointType, float sampling)
{
	...
	pcl::PointCloud<pcl::Normal>::Ptr cloud_normals_t (new pcl::PointCloud<pcl::Normal>);
  pcl::PointCloud<pcl::Normal>::Ptr cloud_normals (new pcl::PointCloud<pcl::Normal>);
	...
	ne.compute (*cloud_normals_t);


  pcl::PointIndices::Ptr nan_indices;
  nan_indices.reset(new pcl::PointIndices());
  pcl::removeNaNNormalsFromPointCloud(*cloud_normals_t, *cloud_normals, nan_indices->indices);

  pcl::ExtractIndices<pcl::PointXYZRGB> extractor;
  extractor.setInputCloud(cloud);
  extractor.setIndices(nan_indices);
